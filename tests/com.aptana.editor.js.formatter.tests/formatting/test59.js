...
if( typeof dojo == "undefined") {
    ...
    */
    ...
    dojo._loadUriAndCheck = function(/*String (URL)*/uri,  /*String*/moduleName,  /*Function?*/cb) {
        ...
        return Boolean(ok && this._loadedModules[moduleName]);
        // Boolean
    }
    dojo.loaded = function() {
        this._loadNotifying = true;
    ...
    dojo.clone = function(/*anything*/o) {
        ...
        if(dojo.isArray(o)) {
            ...
            if(o.nodeType && o.cloneNode) { // isNode
                ...
                // specific to dojo.declare()'d classes!
                for(var i in o) {
                    if(!( i in r) || r[i] != o[i]) {
                        r[i] = dojo.clone(o[i]);
                    }
    ...
    dojo.declare = function(/*String*/className,  /*Function||Array*/ superclass,  /*Object*/ props) {
        ...
        if(mixins) {
            for(var i = 0, m; i < mixins.length; i++) {
                m = mixins[i];
                if(!m) {
                    throw ("Mixin #" + i + " to declaration of " + className + " is null. It's likely a required module is not loaded.")
                };
                superclass = dd._delegate(superclass, m);
            }
        }
        ...
        _core : {
            ...
            inherited : function(name, args, newArgs) {
                ...
                if(this[name] != c || p[name] == c) {
                    mp = this._findMethod(name, c, p, true);
                    if(!mp) {
                        throw (this.declaredClass + ': name argument ("' + name + '") to inherited must match callee (declare.js)');
                    }
                    p = this._findMethod(name, c, mp, false);
                }
                fn = p && p[name];
                // FIXME: perhaps we should throw here?
                if(!fn) {console.debug(mp.declaredClass + ': no inherited "' + name + '" was found (declare.js)');
                    return;
                }
                // if the function exists, invoke it in our scope
                return fn.apply(this, a);
                ...
                _fixEvent : function(evt, sender) {
                    switch(evt.type) {
                        case "keypress":
                            ...
                            if(evt.keyIdentifier == "Enter") {
                                c = 0;
                                // differentiate Enter from CTRL-m (both code 13)
                            } else if((evt.ctrlKey) && (c > 0) && (c < 27)) {
                                c += 96;
                                // map CTRL-[A-Z] codes to ASCII
                            } else if(c == dojo.keys.SHIFT_TAB) {
                                c = dojo.keys.TAB;
                                // morph SHIFT_TAB into TAB + shiftKey: true
                                s = true;
                            } else {
                                c = (c >= 32 && c < 63232 ? c : 0);
                                // avoid generating keyChar for non-printables
                            }
                            return del._synthesizeEvent(evt, {
                                charCode : c,
                                shiftKey : s,
    ...
    if(dojo.isIE || dojo.isOpera) {
        ...
        var _toStyleValue = function(node, type, value) {
            ...
            if(_pixelNamesCache[type] === true) {
                ...
                // (type.indexOf("border") >= 0) ||
                (type.indexOf("padding") >= 0) || (type.indexOf("width") >= 0) || (type.indexOf("height") >= 0) || (type.indexOf("max") >= 0) || (type.indexOf("min") >= 0) || (type.indexOf("offset") >= 0)) {
                    _pixelNamesCache[type] = true;
                    return dojo._toPixelValue(node, value)
                } else {
                    _pixelNamesCache[type] = false;
        ...
        dojo.style = function(/*DomNode|String*/node,  /*String*/style,  /*String?*/value) {
            ...
            var n = dojo.byId(node), args = arguments.length, op = (style == "opacity");
            if(args == 3) {
                return op ? dojo._setOpacity(n, value) : n.style[style] = value;
                /*Number*/
            }
            if(args == 2 && op) {
                return dojo._getOpacity(n);
        ...
        dojo._getPadExtents = function(/*DomNode*/n,  /*Object*/computedStyle) {
            ...
            //		Normally application code will not need to invoke this
            //		directly, and will use the ...box... functions instead.
            var s = computedStyle || gcs(n), px = dojo._toPixelValue, l = px(n, s.paddingLeft), t = px(n, s.paddingTop);
            return {
                l : l,
                t : t,
                w : l + px(n, s.paddingRight),
        ...
        dojo._getBorderExtents = function(/*DomNode*/n,  /*Object*/computedStyle) {
            ...
            //		Normally application code will not need to invoke this
            //		directly, and will use the ...box... functions instead.
            var ne = 'none', px = dojo._toPixelValue, s = computedStyle || gcs(n), bl = (s.borderLeftStyle != ne ? px(n, s.borderLeftWidth) : 0), bt = (s.borderTopStyle != ne ? px(n, s.borderTopWidth) : 0);
            return {
                l : bl,
                t : bt,
                w : bl + (s.borderRightStyle != ne ? px(n, s.borderRightWidth) : 0),
        ...
        dojo._getPadBorderExtents = function(/*DomNode*/n,  /*Object*/computedStyle) {
            ...
            //		Normally application code will not need to invoke this
            //		directly, and will use the ...box... functions instead.
            var s = computedStyle || gcs(n), p = dojo._getPadExtents(n, s), b = dojo._getBorderExtents(n, s);
            return {
                l : p.l + b.l,
                t : p.t + b.t,
                w : p.w + b.w,
        ...
        dojo._getMarginExtents = function(n, computedStyle) {
            ...
            //		Normally application code will not need to invoke this
            //		directly, and will use the ...box... functions instead.
            var s = computedStyle || gcs(n), px = dojo._toPixelValue, l = px(n, s.marginLeft), t = px(n, s.marginTop), r = px(n, s.marginRight), b = px(n, s.marginBottom);
            if(dojo.isSafari && (s.position != "absolute")) {
                // FIXME: Safari's version of the computed right margin
                // is the space between our right edge and the right edge
                // of our offsetParent.
        ...
        dojo._isBodyLtr = function() {
            //FIXME: could check html and body tags directly instead of computed style?  need to ignore case, accept empty values
            return typeof dojo._bodyLtr == "undefined" ? (dojo._bodyLtr = dojo.getComputedStyle(dojo.body()).direction == "ltr") : dojo._bodyLtr;
            // Boolean
        }
        dojo._getIeDocumentElementOffset = function() {
            // summary
            ...
            if(dojo.isIE >= 7) {
                ...
                // IE 6.0
                return {
                    x : dojo._isBodyLtr() || window.parent == window ? de.clientLeft : de.offsetWidth - de.clientWidth - de.clientLeft,
                    y : de.clientTop
                };
                // Object
            }
        ...
        dojo._abs = function(/*HTMLElement*/node,  /*Boolean?*/includeScroll) {
            ...
            if(dojo.isIE) {
                ...
                if(node["offsetParent"]) {
                    ...
                    if(dojo.isSafari &&
                    //(node.style.getPropertyValue("position") == "absolute") &&
                    (gcs(node).position == "absolute") && (node.parentNode == db)) {
                        endNode = db;
                    } else {
                        endNode = db.parentNode;
                    }
    ...
    (function() {
        ...
        dojo.NodeList = function() {
            ...
            if((args.length == 1) && ( typeof args[0] == "number")) {
                ...
                d.forEach(args, function(i) { this.push(i);
                }, this);
            }
        }
        // prototyping subclass for sane browsers
        ...
        if(d.isIE) {
            ...
            var subClassStr = function(className) {
                return (
                    // "parent.dojo.debug('setting it up...'); " +
                    "var a2 = parent." + className + "; " + "var ap = Array.prototype; " + "var a2p = a2.prototype; " + "for(var x in a2p){ ap[x] = a2p[x]; } " + "parent." + className + " = Array; "
                );
            }
            var scs = subClassStr("dojo.NodeList");
            // Hedger's excellent invention
        ...
        dojo.extend(dojo.NodeList, {
            ...
            adopt : function(/*String||Array||DomNode*/queryOrListOrNode,  /*String?*/ position) {
                ...
                // 		or an offset in the childNodes property
                var item = this[0];
                return d.query(queryOrListOrNode).forEach(function(ai) { d.place(ai, item, (position || "last"));
                });
                // dojo.NodeList
            },
            // FIXME: do we need this?
        ...
        // syntactic sugar for DOM events
        dojo.forEach(["mouseover", "click", "mouseout", "mousemove", "blur", "mousedown", "mouseup", "mousemove", "keydown", "keyup", "keypress"], function(evt) {
            var _oe = "on" + evt;
            dojo.NodeList.prototype[_oe] = function(a, b) {
                return this.connect(_oe, a, b);
            }
    ...
    ;(function() {
        ...
        */
        ...
        var buildPath = function(query) {
            var xpath = ".";
            var qparts = query.split(" ");
            // FIXME: this break on span[thinger = foo]
            while(qparts.length) {
                var tqp = qparts.shift();
                var prefix;
                // FIXME: need to add support for ~ and +
        ...
        */
        ...
        var getFilterFunc = function(query) {
            ...
            if(tagName != "*") {
                // tag name match
                ff = agree(ff, function(elem) {
                    return ((elem.nodeType == 1) && (tagName == elem.tagName.toLowerCase())
                    );
                    // return isTn;
                });
            }
        ...
        var pseudos = [{
            ...
            match : function(name, condition) {
                ...
                } else if((condition == "2n") || (condition == "even")) {
                    return function(elem) {
                        return ((getNodeIndex(elem) % 2) == 0);
                    }
                } else if(condition.indexOf("0n+") == 0) {
        ...
        var getStepQueryFunc = function(query) {
            ...
            var sqf = function(root) {
                var qparts = query.split(" ");
                // FIXME: this is an inaccurate tokenizer!
        ...
        var _getQueryFunc = (
            ...
            //		potentially optimized variant or if we should try something
            //		new.
            (document["evaluate"] && !d.isSafari) ? function(query) {
                // has xpath support that's faster than DOM
                var qparts = query.split(" ");
                // can we handle it?
                ...
                // (query.indexOf("=") == -1)
                    ...
                    ((qparts.length > 2) && (query.indexOf(">") == -1)) ||
                    // or something else with moderate complexity. kinda janky
                    (qparts.length > 3) || (query.indexOf("[") >= 0) ||
                    // or if it's a ".thinger" query
                    ((1 == qparts.length) && (0 <= query.indexOf(".")))) {
                        // use get and cache a xpath runner for this selector
                        return getXPathFunc(query);
        ...
        */
        ...
        d._filterQueryResult = function(nodeList, simpleFilter) {
            var tnl = new d.NodeList();
            var ff = (simpleFilter) ? getFilterFunc(simpleFilter) : function() {
                return true;
            };
            // dojo.debug(ff);
            for(var x = 0, te; te = nodeList[x]; x++) {
...
if(!dojo._hasResource["dojo._base.xhr"]) { //_hasResource checks added by build. Do not use _hasResource directly in your code.
    ...
    dojo.formToQuery = function(/*DOMNode||String*/formNode) {
        ...
        //		return URL-encoded string representing the form passed as either a
        //		node or string ID identifying the form to serialize
        return dojo.objectToQuery(dojo.formToObject(formNode));
        // string
    }
    dojo.formToJson = function(/*DOMNode||String*/formNode) {
        // summary:
        //		return a serialized JSON string from a form node or string
        //		ID identifying the form to serialize
        return dojo.toJson(dojo.formToObject(formNode));
        // string
    }
    dojo.queryToObject = function(/*String*/str) {
        // summary:
    ...
    */
    ...
    dojo._contentHandlers = {
        ...
        "json" : function(xhr) {
            if(!djConfig.usePlainJson) {
                console.debug("please consider using a mimetype of text/json-comment-filtered" + " to avoid potential security issues with JSON endpoints" + " (use djConfig.usePlainJson=true to turn off this message)");
            }
            return dojo.fromJson(xhr.responseText);
        },
        "json-comment-optional" : function(xhr) {
        ...
        dojo.xhrPut = function(/*Object*/args) {
            ...
            //		Sends an HTTP PUT request to the server. See dojo.xhrGet() for
            //		a list of commonly accepted properties on args.
            return _doIt("PUT", _makeXhrDeferred(args));
            // dojo.Deferred
        }
        dojo.rawXhrPut = function(/*Object*/args) {
            //	summary:
...
if(!dojo._hasResource["dojo._base.fx"]) { //_hasResource checks added by build. Do not use _hasResource directly in your code.
    ...
    dojo.declare("dojo._Animation", null, {
        ...
        play : function(/*int?*/delay,  /*boolean?*/ gotoStart) {
            ...
            var d = delay || this.delay;
            if(d > 0) {
                setTimeout(dojo.hitch(this, function() { this.play(null, gotoStart);
                }), d);
                return this;
                // dojo._Animation
            }
        ...
        if(dojo.isKhtml && !dojo.isSafari) {
            ...
            dojo._defaultEasing = function(/*Decimal?*/n) {
                ...
                //FIXME: Does this still occur in the supported Safari version?
            }
        } else {
            dojo._defaultEasing = function(/*Decimal?*/n) {
                return 0.5 + ((Math.sin((n + 1.5) * Math.PI)) / 2);
