 ...
 * 
 */
public class FTPConnectionFileManager extends BaseFTPConnectionFileManager
		implements IFTPConnectionFileManager, IPoolConnectionManager {
	...
	private Map<IPath, FTPFile> ftpFileCache = new ExpiringMap<IPath, FTPFile>(
			CACHE_TTL);
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.IFTPConnectionFileManager#init(java.lang.String,
	 * int, org.eclipse.core.runtime.IPath, java.lang.String, char[], boolean,
	 * java.lang.String, java.lang.String, java.lang.String)
	 */
	public void init(String host, int port, IPath basePath, String login,
			char[] password, boolean passive, String transferType,
			String encoding, String timezone) {
		Assert.isTrue(ftpClient == null,
				Messages.FTPConnectionFileManager_already_initialized);
		try {
			...
			this.timezone = timezone != null && timezone.length() == 0 ? null
					: timezone;
			...
			FTPPlugin
					.log(new Status(
							IStatus.WARNING,
							FTPPlugin.PLUGIN_ID,
							Messages.FTPConnectionFileManager_initialization_failed,
							e));
			ftpClient = null;
		}
	}

	protected static void initFTPClient(FTPClient ftpClient, boolean passive,
			String encoding) throws IOException, FTPException {
		ftpClient.setTimeout(TIMEOUT);
		ftpClient.setControlEncoding(encoding);
		ftpClient.setMonitorInterval(1024);
		ftpClient.setConnectMode(passive ? FTPConnectMode.PASV
	...
	private static void connectFTPClient(FTPClient ftpClient)
			throws IOException, FTPException {
		PerformanceStats stats = PerformanceStats.getStats(
				"com.aptana.filesystem.ftp/perf/connect",
	...
	protected void initAndAuthFTPClient(FTPClientInterface clientInterface,
			IProgressMonitor monitor) throws IOException, FTPException {
		if (clientInterface.connected()) {
			return;
		}
		FTPClient newFtpClient = (FTPClient) clientInterface;
		initFTPClient(newFtpClient,
				ftpClient.getConnectMode() == FTPConnectMode.PASV, ftpClient
						.getControlEncoding());
	...
	protected static void setMessageLogger(FTPClient ftpClient,
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.vfs.IConnectionFileManager#connect(org.eclipse
	 * .core.runtime.IProgressMonitor)
	 */
	public void connect(IProgressMonitor monitor) throws CoreException {
		Assert.isTrue(ftpClient != null,
				Messages.FTPConnectionFileManager_not_initialized);
		...
		try {
			...
			if (messageLogWriter == null) {
				if (context != null) {
					...
					if (object instanceof PrintWriter) {
						...
						messageLogWriter = new PrintWriter(
								(OutputStream) object);
					}
				}
				if (messageLogWriter == null) {
				...
				if (messageLogWriter != null) {
					messageLogWriter.println(MessageFormat.format(
							"---------- FTP {0} ----------", host)); //$NON-NLS-1$
					setMessageLogger(ftpClient, messageLogWriter);
				}
			} else {
				messageLogWriter.println(MessageFormat.format(
						"---------- RECONNECTING - FTP {0} ----------", host)); //$NON-NLS-1$
			}

			monitor.beginTask(
					Messages.FTPConnectionFileManager_establishing_connection,
			...
			while (true) {
				...
				if (password.length == 0
						...
						&& (context == null || !context
								.getBoolean(ConnectionContext.NO_PASSWORD_PROMPT))) {
				...
				Policy.checkCanceled(monitor);
				monitor
						.subTask(Messages.FTPConnectionFileManager_authenticating);
				try {
					...
					if (ftpClient.getLastValidReply() == null
						...
						if (context != null
							...
							throw new CoreException(
									new Status(
											...
											MessageFormat
													.format(
															Messages.FTPConnectionFileManager_FailedAuthenticate,
															e
																	.getLocalizedMessage()),
											e));
						}
						promptPassword(
								MessageFormat
										.format(
												Messages.FTPConnectionFileManager_ftp_auth,
												host),
			...
			ftpClient.setType(IFTPConstants.TRANSFER_TYPE_ASCII
					.equals(transferType) ? FTPTransferType.ASCII
					: FTPTransferType.BINARY);

			if ((hasServerInfo || (context != null && context
					.getBoolean(ConnectionContext.QUICK_CONNECT)))
					&& !(context != null && context
							.getBoolean(ConnectionContext.DETECT_TIMEZONE))) {
				return;
			}
			...
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID,
					Messages.FTPConnectionFileManager_HostNameNotFound
							+ e.getLocalizedMessage(), e));
		} catch (FileNotFoundException e) {
			safeQuit();
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID,
					Messages.FTPConnectionFileManager_RemoteFolderNotFound
							+ e.getLocalizedMessage(), e));
		} catch (Exception e) {
			safeQuit();
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID,
					Messages.FTPConnectionFileManager_connection_failed
							+ e.getLocalizedMessage(), e));
		} finally {
			monitor.done();
		}
	...
	protected void getherServerInfo(ConnectionContext context,
			IProgressMonitor monitor) {
		Policy.checkCanceled(monitor);
		monitor
				.subTask(Messages.FTPConnectionFileManager_gethering_server_info);
		try {
			serverFeatures = null;
		...
		if (context != null
				&& context.getBoolean(ConnectionContext.DETECT_TIMEZONE)) {
			serverTimeZoneShift = Integer.MIN_VALUE;
		} else if (timezone != null) {
			TimeZone tz = TimeZone.getTimeZone(timezone);
			if (tz != null) {
				long time = new Date().getTime();
				serverTimeZoneShift = TimeZone.getDefault().getOffset(time)
						- tz.getOffset(time);
			}
		}
		if (serverTimeZoneShift == Integer.MIN_VALUE) {
			...
			try {
					...
					for (FTPFile ftpFile : rootFiles) {
						if (ftpFile.isFile()
								...
								&& !(ftpFile.lastModified().getHours() == 0
										&& ftpFile.lastModified().getMinutes() == 0 && ftpFile
				...
				if (file == null) {
					...
					ftpClient.put(new ByteArrayInputStream(new byte[] {}),
							TMP_TIMEZONE_CHECK);
				...
				if (file != null) {
					if (serverSupportsFeature("MDTM")) { //$NON-NLS-1$
						Date lastModifiedLocalTZ = ftpClient.modtime(file
								.getName());
						if (lastModifiedLocalTZ != null) {
							// align to minutes
							serverTimeZoneShift = (lastModifiedLocalTZ
									.getTime() - lastModifiedLocalTZ.getTime() % 60000)
									- (lastModifiedServerInLocalTZ.getTime() - lastModifiedServerInLocalTZ
											.getTime() % 60000);
						}
					}
					if (serverTimeZoneShift == Integer.MIN_VALUE) {
						serverTimeZoneShift = (lastModifiedLocal.getTime() - lastModifiedLocal
								.getTime() % 60000)
								- (lastModifiedServerInLocalTZ.getTime() - lastModifiedServerInLocalTZ
										.getTime() % 60000);
						// align to 1/4 hour
					...
					if (context != null) {
						...
						int rawOffset = (int) (cal.get(Calendar.ZONE_OFFSET)
								+ cal.get(Calendar.DST_OFFSET) - serverTimeZoneShift);
						context.put(ConnectionContext.SERVER_TIMEZONE, TimeZone
			...
			if (serverTimeZoneShift == Integer.MIN_VALUE) {
				Calendar cal = Calendar.getInstance();
				serverTimeZoneShift = cal.get(Calendar.ZONE_OFFSET)
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.vfs.IConnectionFileManager#disconnect(org.eclipse
	 * .core.runtime.IProgressMonitor)
	 */
	public synchronized void disconnect(IProgressMonitor monitor)
		...
		monitor = Policy.monitorFor(monitor);
		monitor.beginTask(Messages.FTPConnectionFileManager_closing_connection,
				IProgressMonitor.UNKNOWN);
		try {
			ftpClient.quit();
		} catch (Exception e) {
	...
	protected void changeCurrentDir(IPath path) throws FTPException,
			IOException {
		try {
			if (cwd == null) {
	...
	private static void fillFileInfo(ExtendedFileInfo fileInfo, FTPFile ftpFile) {
		...
		fileInfo.setLastModified(ftpFile.lastModified() != null ? ftpFile
				.lastModified().getTime() : 0);
		fileInfo.setOwner(ftpFile.getOwner());
		fileInfo.setGroup(ftpFile.getGroup());
		fileInfo.setPermissions(Policy.permissionsFromString(ftpFile
				.getPermissions()));
		if (ftpFile.isLink()) {
			fileInfo.setAttribute(EFS.ATTRIBUTE_SYMLINK, true);
			fileInfo.setStringAttribute(EFS.ATTRIBUTE_LINK_TARGET, ftpFile
					.getLinkedName().trim());
		}
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#clearCache(org.eclipse
	 * .core.runtime.IPath)
	 */
	@Override
	protected void clearCache(IPath path) {
		...
		for (IPath p : new ArrayList<IPath>(ftpFileCache.keySet())) {
			if (p.segmentCount() >= segments
					&& path.matchingFirstSegments(p) == segments) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#checkConnected()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#getRootCanonicalURI
	 * ()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#fetchFile(org.eclipse
	 * .core.runtime.IPath, int, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@SuppressWarnings("deprecation")
	@Override
	protected ExtendedFileInfo fetchFile(IPath path, int options,
		...
		try {
				...
				for (FTPFile ftpFile : ftpFiles) {
					...
					if (".".equals(ftpFile.getName()) || "..".equals(ftpFile.getName())) { //$NON-NLS-1$ //$NON-NLS-2$
						if (Path.ROOT.equals(path)
								&& ".".equals(ftpFile.getName())) { //$NON-NLS-1$
					...
					ftpFileCache
							.put(dirPath.append(ftpFile.getName()), ftpFile);
					if (name != null
							&& name.equalsIgnoreCase(ftpFile.getName())) {
						result = ftpFile;
					}
				}
			}
			if ((options & IExtendedFileStore.DETAILED) != 0) {
				if (result != null && !result.isDir() && name != null
			...
			if (result == null && Path.ROOT.equals(path)) {
				result = new FTPFile(
						"", path.toPortableString(), 0, true, new Date(0)); //$NON-NLS-1$
			}
			if (result != null) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#fetchFiles(org.eclipse
	 * .core.runtime.IPath, int, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@SuppressWarnings("deprecation")
	@Override
	protected ExtendedFileInfo[] fetchFiles(IPath path, int options,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		...
		try {
			...
			monitor.beginTask(
					Messages.FTPConnectionFileManager_gethering_file_details,
					ftpFiles.length);
				...
				if ((options & IExtendedFileStore.DETAILED) != 0) {
					if (!ftpFile.isDir()
						...
						if (serverSupportsFeature("MDTM")) { //$NON-NLS-1$
							...
							Policy.checkCanceled(monitor);
							try {
								Date lastModifiedLocalTZ = ftpClient
										.modtime(ftpFile.getName());
								if (lastModifiedLocalTZ != null) {
									ftpFile
											.setLastModified(lastModifiedLocalTZ);
								}
							} catch (FTPException e) {
							}
						}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#listDirectory(org
	 * .eclipse.core.runtime.IPath, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected String[] listDirectory(IPath path, IProgressMonitor monitor)
		...
		try {
			...
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID,
					Messages.FTPConnectionFileManager_listing_directory_failed,
					e));
		} finally {
			monitor.done();
		}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#readFile(org.eclipse
	 * .core.runtime.IPath, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected InputStream readFile(IPath path, IProgressMonitor monitor)
			throws CoreException, FileNotFoundException {
		monitor.beginTask(
				Messages.FTPConnectionFileManager_initiating_download, 4);
		...
		try {
			...
			downloadFtpClient.setType(IFTPConstants.TRANSFER_TYPE_ASCII
					.equals(transferType) ? FTPTransferType.ASCII
					: FTPTransferType.BINARY);
			try {
				downloadFtpClient.chdir(path.removeLastSegments(1)
						.toPortableString());
			} catch (FTPException e) {
			...
			try {
				return new FTPFileDownloadInputStream(pool, downloadFtpClient,
						new FTPInputStream(downloadFtpClient, path
								.lastSegment()));
			} catch (FTPException e) {
				throwFileNotFound(e, path);
				return null;
			...
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID,
					Messages.FTPConnectionFileManager_opening_file_failed, e));
		} finally {
			monitor.done();
		}
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#writeFile(org.eclipse
	 * .core.runtime.IPath, long, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected OutputStream writeFile(IPath path, long permissions,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		monitor.beginTask(
				Messages.FTPConnectionFileManager_initiating_file_upload, 4);
		...
		try {
			...
			uploadFtpClient.setType(IFTPConstants.TRANSFER_TYPE_ASCII
					.equals(transferType) ? FTPTransferType.ASCII
					: FTPTransferType.BINARY);
			IPath dirPath = path.removeLastSegments(1);
			try {
			...
			return new FTPFileUploadOutputStream(pool, uploadFtpClient,
					new FTPOutputStream(uploadFtpClient,
							generateTempFileName(path.lastSegment())), path
							.lastSegment(), null, permissions);
		} catch (Exception e) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#deleteDirectory(
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#deleteFile(org.eclipse
	 * .core.runtime.IPath, org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void deleteFile(IPath path, IProgressMonitor monitor)
		...
		try {
			...
			throw new CoreException(new Status(Status.ERROR,
					FTPPlugin.PLUGIN_ID, MessageFormat.format(
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#createDirectory(
	 * org.eclipse.core.runtime.IPath,
	 * org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void createDirectory(IPath path, IProgressMonitor monitor)
		...
		try {
			try {
				...
				changeFilePermissions(path, PreferenceUtils
						.getDirectoryPermissions(), monitor);
			} catch (FTPException e) {
				throwFileNotFound(e, path);
			}
		} catch (FileNotFoundException e) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#renameFile(org.eclipse
	 * .core.runtime.IPath, org.eclipse.core.runtime.IPath,
	 * org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void renameFile(IPath sourcePath, IPath destinationPath,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		try {
			changeCurrentDir(Path.ROOT);
			Policy.checkCanceled(monitor);
			try {
				ftpClient.rename(sourcePath.toPortableString(), destinationPath
						.toPortableString());
			} catch (FTPException e) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#setModificationTime
	 * (org.eclipse.core.runtime.IPath, long,
	 * org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void setModificationTime(IPath path, long modificationTime,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		if (!serverSupportsFeature("MFMT") && !setUTimeSupported) { //$NON-NLS-1$
			return;
		}

		try {
			if (serverSupportsFeature("MFMT")) {
				setMFMTTime(path, modificationTime, monitor);
			} else if (setUTimeSupported) {
				setUTime(path, modificationTime, monitor);
			}
		} catch (Exception e) {
			throw new CoreException(
					new Status(
	...
	/**
	 * Change modification time for a remote file using MFMT method
	 * 
	 * @param path
	 *            path to file
	 * @param modificationTime
	 *            file modification time to set
	 * @param monitor
	 *            progress monitor
	 * @throws IOException
	 * @throws FTPException
	 */
	protected void setMFMTTime(IPath path, long modificationTime,
			IProgressMonitor monitor) throws IOException, FTPException {
		try {
			IPath dirPath = path.removeLastSegments(1);
			changeCurrentDir(dirPath);
			Policy.checkCanceled(monitor);
			ftpClient
					.setModTime(path.lastSegment(), new Date(modificationTime));
		} catch (FileNotFoundException e) {
			throw e;
		} catch (OperationCanceledException e) {
			throw e;
	...
	/**
	 * Change modification time for a remote file using SITE UTIME method
	 * 
	 * @param path
	 *            path to file
	 * @param modificationTime
	 ...
	 * @throws FTPException
	 */
	protected void setUTime(IPath path, long modificationTime,
			IProgressMonitor monitor) throws IOException, FTPException {
		...
		try {
			...
			SimpleDateFormat tzFormat = new SimpleDateFormat("yyyyMMddHHmmss");

			long tzOffset = (long) tz.getOffset(new Date().getTime());

			String time = tzFormat.format(modificationTime - tzOffset);

			// Set time using SITE UTIME command. Time must be in UTC.
			ftpClient.site("UTIME " + path.lastSegment() + " " + time + " "
					+ time + " " + time + " UTC");
		} catch (FTPException e) {
	...
	//
	// protected void setmodtime(long modtime, String remoteFile, TimeZone
	// timezone) throws IOException, FTPException {
	// tsFormat.setTimeZone(timezone);
	// if(this.setModtimeSupported) {
	// try {
	// FTPReply reply = ftpClient..sendCommand("MDTM "+tsFormat.format(new
	// Date(modtime))+" "+remoteFile);
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#changeFilePermissions
	 * (org.eclipse.core.runtime.IPath, long,
	 * org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void changeFilePermissions(IPath path, long permissions,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		...
		try {
			...
			changeCurrentDir(dirPath);
			Policy.checkCanceled(monitor);
			chmodSupported = ftpClient
					.site("CHMOD " + Long.toOctalString(permissions) + " " + path.lastSegment()); //$NON-NLS-1$ //$NON-NLS-2$
		} catch (FileNotFoundException e) {
			throw e;
		} catch (OperationCanceledException e) {
			throw e;
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.ftp.BaseFTPConnectionFileManager#changeFileGroup(
	 * org.eclipse.core.runtime.IPath, java.lang.String,
	 * org.eclipse.core.runtime.IProgressMonitor)
	 */
	@Override
	protected void changeFileGroup(IPath path, String group,
			IProgressMonitor monitor) throws CoreException,
			FileNotFoundException {
		...
		try {
			...
			changeCurrentDir(dirPath);
			Policy.checkCanceled(monitor);
			chgrpSupported = ftpClient
					.site("CHGRP " + group + " " + path.lastSegment()); //$NON-NLS-1$ //$NON-NLS-2$
		} catch (FileNotFoundException e) {
			throw e;
		} catch (OperationCanceledException e) {
			throw e;
	...
	private FTPFile[] ftpSTAT(String dirname) throws IOException, FTPException,
			ParseException {
		if (fileFactory == null) {
			try {
	...
	private FTPFile[] listFiles(IPath dirPath, IProgressMonitor monitor)
			throws IOException, ParseException, FTPException {
		FTPFile[] ftpFiles = null;
		if (statSupported != Boolean.FALSE) {
			try {
				ftpFiles = ftpSTAT(dirPath.addTrailingSeparator()
						.toPortableString());
			} catch (FTPException e) {
		...
		if (fileFactory.getSystem().toUpperCase().startsWith(WINDOWS_STR)
				&& ftpFiles != null) {
			for (FTPFile ftpFile : ftpFiles) {
	...
	private void recursiveDeleteTree(IPath path, IProgressMonitor monitor,
			MultiStatus status) throws IOException, ParseException {
		try {
			changeCurrentDir(path);
			FTPFile[] ftpFiles = listFiles(path, monitor);
