...
public class CSSCodeScanner extends BufferedRuleBasedScanner
{
	...
	{
		IToken keyword = createToken("keyword.other.unit.css");
		IToken ids = createToken("entity.other.attribute-name.id.css");
		IToken classes = createToken("entity.other.attribute-name.class.css");
		IToken rule = createToken("keyword.control.at-rule.media.css"); // FIXME
		// name
		// of
		// keyword
		// should
		...
		rules.add(wordRule);
		rules.add(new RegexpRule("(#)([0-9a-fA-F]{3}|[0-9a-fA-F]{6})\\b",
				createToken("constant.other.color.rgb-value.css")));
		rules.add(new RegexpRule("#[_a-z0-9-]+", ids));
		rules.add(new RegexpRule("\\.[_a-z0-9-]+", classes));
		rules.add(new RegexpRule("@[_a-z0-9-]+", rule));

		rules.add(new RegexpRule("(\\-|\\+)?\\s*[0-9]+(\\.[0-9]+)?", createToken("constant.numeric.css")));
		...
		rules
				.add(new RegexpRule(
						...
						createToken("entity.name.tag.css")));

		rules.add(new RegexpRule("\\b(all|aural|braille|embossed|handheld|print|projection|screen|tty|tv)\\b",
				createToken("support.constant.media.css")));

		rules.add(new RegexpRule("%", createToken("keyword.other.unit.css")));
		rules
				.add(new RegexpRule(
						...
						createToken("support.constant.property-value.css")));
		rules
				.add(new RegexpRule(
						...
						createToken("support.type.property-name.css")));
		rules
				.add(new RegexpRule(
						...
						createToken("support.constant.font-name.css")));
