...
A command can optionally have a key binding associated with it. These key bindings are specified using an array of KEY_BINDING_VALUEs, like so:

bc. command.key_binding = [ KEY_BINDING_VALUE, KEY_BINDING_VALUE, etc. ]

There should be *at least two* KEY_BINDING_VALUEs supplied, one of which is a CONTROL_KEY_SPECIFIER:

* :control
* :option
* :meta
* :shift
* :command, :windows_key - these are platform and/or hardware specific synonyms for :meta

We strongly recommend against the use of :SHIFT without including another CONTROL_KEY_SPECIFIER, as the results can be unpredictable.

For other keys, the values can be specified using their Latin-1 character (e.g. "q" or "#") or using a numeric keycode. Keycodes are not recommended as they aren't portable, but they may not be avoidable in all cases. If a key is specified using an upper case character or a symbol that requires the SHIFT key to be held down, the :SHIFT modifier is implied. Different platforms may display such key bindings differently in menus: some might display a key binding specified as "[:control, "#"]" as "Control-Shift-3", while others would display "Control-#".

It may be necessary for commands to define specific key bindings for specific platforms. To enable this, the @key_binding@ property supports hash-table style access for setting platform specific key bindings. Platform-specific bindings always take precedence over the generic binding. See the example below:
...
FORTHCOMING: special symbol definitions for other keys (arrows, function keys, return, enter, etc.)
