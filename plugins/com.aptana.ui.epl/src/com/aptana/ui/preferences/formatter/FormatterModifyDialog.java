...
import com.aptana.ui.preferences.formatter.internal.FormatterControlManager;
import com.aptana.ui.preferences.formatter.internal.FormatterDialogPreferences;
...
public abstract class FormatterModifyDialog extends StatusDialog implements
	...
	protected IStatus doValidate() {
		...
		if (!name.equals(profile.getName()) && manager.containsName(name)) {
			return new Status(IStatus.ERROR, UIEplPlugin.PLUGIN_ID,
					FormatterMessages.FormatterModifyDialog_nameExists);
		}

		if (profile.isBuiltInProfile()) {
			return new Status(IStatus.INFO, UIEplPlugin.PLUGIN_ID,
