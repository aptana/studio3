...
public abstract class OptionsConfigurationBlock
{
	...
	protected void validateValuePresenceFor(PreferenceKey key)
	{
		...
		{
			UIEplPlugin.logError("preference option missing: " + key + " (" + this.getClass().getName() + ')'); //$NON-NLS-1$//$NON-NLS-2$
		}
	}

	private int getRebuildCount()
	{
		return fManager.getWorkingCopy(new DefaultScope().getNode(UIEplPlugin.PLUGIN_ID)).getInt(REBUILD_COUNT_KEY, 0);
	}

	private void incrementRebuildCount()
	{
		fRebuildCount++;
		fManager.getWorkingCopy(new DefaultScope().getNode(UIEplPlugin.PLUGIN_ID)).putInt(REBUILD_COUNT_KEY,
				fRebuildCount);
	}
	...
	protected boolean processChanges(IWorkbenchPreferenceContainer container)
	{
		...
		else
		{
			...
			{
				UIEplPlugin.logError(e.getMessage(), e);
				return false;
			}
			if (doBuild)
			{
