 ...
 * 
 */
/* package */final class ConnectionPointManager extends PlatformObject
	...
	private List<ConnectionPoint> connections = Collections
			.synchronizedList(new ArrayList<ConnectionPoint>());
	...
	private List<IMemento> unresolvedConnections = Collections
			.synchronizedList(new ArrayList<IMemento>());
		...
		if (file.exists()) {
			...
			try {
				...
				for (IMemento child : memento.getChildren(ELEMENT_CONNECTION)) {
					ConnectionPoint connectionPoint = restoreConnectionPoint(
							child, null);
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#addConnectionPoint(com
	...
	public void addConnectionPoint(IConnectionPoint connectionPoint) {
		...
		if (!connections.contains(connectionPoint)) {
			...
			dirty = true;
			broadcastEvent(new ConnectionPointEvent(this,
					ConnectionPointEvent.POST_ADD, connectionPoint));
		}
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#removeConnectionPoint(
	...
	public void removeConnectionPoint(IConnectionPoint connectionPoint) {
		if (connections.contains(connectionPoint)) {
			...
			dirty = true;
			broadcastEvent(new ConnectionPointEvent(this,
					ConnectionPointEvent.POST_DELETE, connectionPoint));
		}
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#connectionPointChanged
	 * (com.aptana.ide.core.io.IConnectionPoint)
	 */
	public void connectionPointChanged(IConnectionPoint connectionPoint) {
		if (connections.contains(connectionPoint)) {
			dirty = true;
			broadcastEvent(new ConnectionPointEvent(this,
					ConnectionPointEvent.POST_CHANGE, connectionPoint));
		}
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#cloneConnectionPoint(com
	 * .aptana.ide.core.io.IConnectionPoint)
	 */
	public IConnectionPoint cloneConnectionPoint(
			IConnectionPoint connectionPoint) throws CoreException {
		Assert.isLegal(connectionPoint instanceof ConnectionPoint);
		IMemento memento;
		try {
			...
			throw new CoreException(
					new Status(
							IStatus.ERROR,
							CoreIOPlugin.PLUGIN_ID,
							Messages.ConnectionPointManager_FailedStoreConnectionProperties,
							e));
		}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#getType(java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#getType(com.aptana.ide
	 * .core.io.IConnectionPoint)
	 */
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#getConnectionPointCategories
	 * ()
	 */
	public IConnectionPointCategory[] getConnectionPointCategories() {
		return categories.values().toArray(
				new IConnectionPointCategory[categories.size()]);
	}
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#getConnectionPointCategory
	 * (String)
	 */
	...
	private ConnectionPoint restoreConnectionPoint(IMemento memento, String id)
		...
		if (typeId != null) {
			...
			if (element != null) {
				...
				if (object instanceof ConnectionPoint) {
					...
					connectionPoint.setId((id != null) ? id : memento
							.getString(ATTR_ID));
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#restore15ConnectionPoint
	 * (java.lang.String, java.lang.String)
	 */
	public ConnectionPoint restore15ConnectionPoint(String type, String data)
			throws CoreException {
		ConnectionPoint connectionPoint = null;
		String typeId = null;
		if (type.equals("com.aptana.ide.core.ui.io.file.LocalFileManager")) { //$NON-NLS-1$
			typeId = "local"; //$NON-NLS-1$
				...
				.equals("com.aptana.ide.core.ui.io.file.ProjectFileManager")) { //$NON-NLS-1$
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#createConnectionPoint(
	 * com.aptana.ide.core.io.ConnectionPointType)
	 */
	public IConnectionPoint createConnectionPoint(ConnectionPointType type)
			throws CoreException {
		if (type != null) {
	 ...
	 * java.lang.String)
	 */
	public IConnectionPoint createConnectionPoint(String typeId)
			throws CoreException {
		ConnectionPoint connectionPoint = null;
		IConfigurationElement element = configurationElements.get(typeId);
		if (element != null) {
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#addConectionPointListener
	 ...
	 * 
	 * @see
	 * com.aptana.ide.core.io.IConnectionPointManager#removeConnectionPointListener
	 * (com.aptana.ide.core.io.IConnectionPointListener)
	 */
	...
	private void readExtensionRegistry() {
		EclipseUtil.processConfigurationElements(CoreIOPlugin.PLUGIN_ID,
				EXTENSION_POINT_ID, new IConfigurationElementProcessor() {
	...
	private void readElement(IConfigurationElement element) {
		if (TAG_CONNECTION_POINT_CATEGORY.equals(element.getName())) {
			...
			boolean remote = Boolean.parseBoolean(element
					.getAttribute(ATT_REMOTE));
			...
			if (category == null) {
				...
				if (category == null) {
					categories
							.put(defaultCategoryId,
									category = new ConnectionPointCategory(
											defaultCategoryId,
											Messages.ConnectionPointManager_CategoryUnknown,
											Integer.MAX_VALUE));
				}
			}
			ConnectionPointType type = new ConnectionPointType(typeId, name,
					category);
			types.add(type);
